...
- **Full corpus**: ~8-12M papers
- **Estimated chunks**: ~240M (avg ~20-30 chunks/paper)
- **Estimated storage**: ~528 GB in Qdrant (384-dim vectors + payloads)

### V0-Alpha: Test Sample (100K papers)

First target a **100,000 paper sample** to validate the full pipeline end-to-end before scaling:
- Same filters (OA, 2015+, English, >10 citations)
- Ordered by citation count descending (highest quality first)
- **Estimated chunks**: ~2.5M
- **Estimated storage**: ~5.5 GB in Qdrant
- **Goal**: Validate chunking quality, embedding pipeline, search API, and benchmark fulltext vs abstract-only retrieval on a manageable subset
...
4. **Chunker** - Hybrid section-aware chunking with two tiers:
   - **Tier 1 (short sections)**: If section length ≤ `MAX_SECTION_CHARS` (default 3000 chars), chunk on sentence boundaries with max chunk size 1000 chars. Fast, no LLM needed.
   - **Tier 2 (long sections)**: If section length > `MAX_SECTION_CHARS`, use **semantic chunking via LLM**. Sends the section text to an OSS model on Chutes (e.g. `openai/gpt-oss-120b-TEE`) asking it to split the text into semantically coherent chunks of ~800-1200 chars each. Falls back to sentence-boundary splitting if LLM call fails.
   - Preserves section boundaries (never splits across sections)
   - Attaches metadata: section name, section type, order, paper ID, chunking method (`sentence_split` or `semantic`)
...
```python
class HybridChunker:
    """Hybrid section-aware chunker: sentence splitting for short sections,
    LLM-based semantic chunking for long sections."""

    MAX_SECTION_CHARS = 3000  # Threshold to trigger semantic chunking

    def __init__(self, max_chunk_size: int = 1000,
                 chutes_api_key: str = None,
                 semantic_model: str = "openai/gpt-oss-120b-TEE"):
        ...
    ...
    def _chunk_section(self, section: Section) -> List[str]:
        ...
        if len(section.content) <= self.MAX_SECTION_CHARS:
            return self._sentence_split(section)
        else:
            return self._semantic_chunk(section)

    def _sentence_split(self, section: Section) -> List[str]:
        """Split section text on sentence boundaries (Tier 1)."""
        ...

    def _semantic_chunk(self, section: Section) -> List[str]:
        """Use LLM via Chutes API to semantically chunk long sections (Tier 2).
        Falls back to _sentence_split on failure."""
        ...
```
...
chunker:
  max_chunk_size: 1000
  max_section_chars: 3000       # Trigger semantic chunking above this
  semantic_model: "openai/gpt-oss-120b-TEE"  # OSS model via Chutes
...
pipeline:
  ...
  max_papers: 100000           # V0-Alpha: test sample limit
  log_level: "INFO"
```
...
- Set up repo structure, `.env`, `requirements.txt`, config
- Implement `discovery.py` (ES queries, 100k limit, sorted by citations desc)
- Implement `downloader.py` (async PDF fetch)
- Implement `extractor.py` (GROBID TEI parser)
- Implement `chunker.py` (hybrid: sentence split + Chutes LLM semantic chunking)
- Implement `embedder.py`, `storage.py`
- Implement `state.py` for checkpointing

### Phase 2: Orchestrator + CLI
- Implement `orchestrator.py`
- Build `scripts/run_pipeline.py` CLI with `--max-papers 100000` flag
- Docker compose for GROBID + Qdrant
- End-to-end test with 100 papers, then 100K alpha run
...
### Phase 4: Validation
- Run benchmark: fulltext vs abstract-only on 100K sample
- Analyze results, tune chunking thresholds if needed
- If alpha results are good, proceed to full scale run
...
pydantic>=2.0.0
requests>=2.31.0
openai>=1.0.0              # For Chutes API (OpenAI-compatible) semantic chunking
pyyaml>=6.0
tqdm>=4.66.0
numpy>=1.26.0
```
...
```
ES_HOST=https://babel.desci.com/es
ES_USER=elastic
ES_PWD=SNys0vyXoFjEJj_NeAD2
ES_INDEX=works_opt
CHUTES_API_KEY=cpk_54bea64b6d434a808d2c4b7c313bd535.264495e656ef5b23b90fe45d396ae1a0.5WbqSnmtdaxUj5pt5BiTiBEm6116lnwG
```
